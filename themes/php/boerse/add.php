...
if(isset($_POST['add'])){
	...
	$hour_unit = $_POST['hour_unit'];
	$price = $_POST['price'];
	$comment = $_POST['comment'];
	$uid = $userdata['id'];
	
	if($subject != "" || $hour_unit != "" || $price != ""){
		$pdo->insert('INSERT INTO offers (uid, subject, hour_unit, price, comment) VALUES ("'.$uid.'","'.$subject.'","'.$hour_unit.'","'.$price.'","'.$comment.'")');
		setcookie("message", "<div class=\"alert alert-success\" role=\"alert\">Der Eintrag wurde erfolgreich erstellt!</div>", time()+60, "/", "nachhilfe.staddi99.ovh");
